...
"); // add empty line at the end -> looks better.
            ...
            if (match.Success)
            {
                ...
                if (!ident2 && !ship2 && !character2 && !asteroid2 && !planet2 && !pos2 && !word2)
                {
                    ...
                    if (ident1)
                    {
                        ...
                        {
                            MovePlayerPilotToPlayer(player, (IMyIdentity)CommandIdentify.IdentifyCache, safely, true);
                    ...
                    if (pos1)
                    {
                        ...
                        MovePlayerPilotToPosition(player, position, safely);
                        CommandTeleportBack.SaveTeleportInHistory(currentPosition);
                        return true;
                    }
                    if (gps1)
                    ...
                    if (character1)
                    {
                        ...
                        if (Int32.TryParse(match.Groups["Character1"].Value.Substring(1), out index) && index > 0 && index <= CommandPlayerStatus.IdentityCache.Count)
                        {
                            ...
                            MovePlayerPilotToPlayer(player, selectedPlayer, safely, true);
                            CommandTeleportBack.SaveTeleportInHistory(currentPosition);
                            return true;
                        }
                    }
        ...
        private void Move(IMyEntity source, IMyEntity target, bool safely)
        {

        }

        private void MovePlayerPilotToPosition(IMyPlayer sourcePlayer, Vector3D targetPosition, bool safely)
        {
        ...
        private void MovePlayerPilotToPlayer(IMyPlayer sourcePlayer, IMyIdentity targetedPlayerIdentity, bool safely, bool agressivePosition)
        {
            var listplayers = new List<IMyPlayer>();
            MyAPIGateway.Players.GetPlayers(listplayers, p => p.PlayerID == targetedPlayerIdentity.PlayerId);
            MovePlayerPilotToPlayer(sourcePlayer, listplayers.FirstOrDefault(), safely, agressivePosition);
        }
