    ...
    public class CommandShipOff : ChatCommand
    {
        ...
        public override bool Invoke(ulong steamId, long playerId, string messageText)
        {
            if (messageText.Equals("/off", StringComparison.InvariantCultureIgnoreCase))
            {
                ...
                if (shipEntity != null)
                {
                    MessageSyncGridChange.SendMessage(SyncGridChangeType.SwitchOffPower, shipEntity.EntityId, null, MyAPIGateway.Session.Player.PlayerID);
                    return true;
                }
            ...
            if (match.Success)
            {
                ...
                MessageSyncGridChange.SendMessage(SyncGridChangeType.SwitchOffPower, 0, shipName, MyAPIGateway.Session.Player.PlayerID);
                return true;
            }
